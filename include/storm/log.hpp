...
#  define STORM_LOG(logger, severity, format, ...) \
    storm::log(logger, {}, static_cast<int>(severity), format, ##__VA_ARGS__)
#else
#  define STORM_LOG(logger, severity, format, ...) \
    storm::log(logger, STORM_CURRENT_SOURCE_LOCATION(), static_cast<int>(severity), format, ##__VA_ARGS__)
#endif
